 ...
 * Main JS file
 *
 * @author: Adel <adelal@metropolia.fi>
...
const renderMenu = (menu, title, targetElem) => {
  const list1 = document.querySelectorAll('.menu-item-price');
  const h3 = document.querySelector('.menu-group-heading');
  h3.textContent = title;
  const list = targetElem;
  let i = 0;
  for (const dish of menu[0]) {
    list[i].textContent = dish;
    i++;
  }
  let j = 0;
  for (const dish of menu[1]) {
    list1[j].textContent = dish;
    j++;
  }
};
...
const renderAllMenus = async () => {
  const menuWrapper = document.querySelectorAll('.menu-item-description');
