...
const getDailyMenu = async (restaurantId, lang) => {
  try {
    ...
    const titleEn = Object.values(menu.courses).map((course) => course.title_en);
    const price = Object.values(menu.courses).map((course) => course.price);
    const coursesEn = [titleEn, price];
    const titleFi = Object.values(menu.courses).map((course) => course.title_fi);
    const coursesFi = [titleFi, price];
